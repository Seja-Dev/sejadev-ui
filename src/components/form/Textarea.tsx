...
export function Textarea({
  ...
  if (skeleton)
    return (
      <div
        className={`skeleton w-full h-24 rounded-md ${skeletonClassName}`}></div>
    )

  return (
    <div className="relative flex flex-col">
      <textarea
        className={`${fullWidth && 'w-full'} ${className} resize-none border-2 peer border-base-100 rounded-md bg-[#242424] pt-5 p-2 pl-5 text-base text-base-content outline-none ${error ? `focus:border-error focus:ring-1 focus:ring-error` : 'focus:border-primary focus:ring-1 focus:ring-primary'}`}
        placeholder=""
        name={name}
        value={value}
        rows={rows}
